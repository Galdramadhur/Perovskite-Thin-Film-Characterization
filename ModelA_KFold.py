...
for train, test in kfold.split(inputs1,inputs2, targets):
    ...
    early_stopping_callback = tensorflow.keras.callbacks.EarlyStopping(monitor='val_loss',patience=30)
    
    # Fit data to model
    history = model_kfold.fit([inputs1[train],inputs2[train]], targets[train], epochs=100, validation_data=([inputs1[test],inputs2[test]], targets[test]),batch_size = 8, verbose = 1, callbacks=[model_checkpoint_callback,early_stopping_callback])
    
    model_kfold.load_weights(checkpoint_filepath)
    model_kfold.save(Results_path + '/Models' + "/KFoldA_" + Date + "_" +str(fold_no))
