...
#Beginning of AUAC pipeline, first read in all UV-Vis data
os.chdir(UVVis_path)
...
#Divide UV-Vis by thicknesses; state_thickness_division ensure this only done once
...
#Calculate AUAC using simpsons integrals
area_list = []   
for i in range(len(UVV_list)):
...
#Compute average AUAC, then normalize and duplicate the list for flip=1
avg_area_list = []
for i in range(len(area_list)//3):
    avg_area=(area_list[3*i]+area_list[3*i+1]+area_list[3*i+2])/3
    avg_area_list.append(avg_area)

avg_area_max = np.max(avg_area_list,axis=-1)
avg_area_min = np.min(avg_area_list,axis=-1)
avg_area_list = (avg_area_list-avg_area_min)/(avg_area_max-avg_area_min)+1


avg_area_list_aug = np.repeat(avg_area_list,2)
...
data_list_type = avg_area_list_aug
...
#Seed values are fixed to improve consistency during training; further shuffling can be done via the KFold Argument
seed_value = 1
seed(seed_value)
...
for train, test in kfold.split(inputs1,inputs2, targets):
    ...
    model_kfold.save(Results_path + '/Models' + "/KFoldC _" + Date + "_" +str(fold_no))
