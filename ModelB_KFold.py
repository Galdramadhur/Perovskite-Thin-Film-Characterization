...
split_top_image_file_list,split_top_image_list,split_back_image_file_list,split_back_image_list,rows,flip = load_split_flip(rows=2,flip=1)
...
#Min-Max norm for the solo list, plus 1
peak_max=np.max(pl_peak_list_solo, axis=-1)
peak_min=np.min(pl_peak_list_solo,axis=-1)
for i in range(len(pl_peak_list_solo)):
    pl_peak_list_solo[i]=(pl_peak_list_solo[i]-peak_min)/(peak_max-peak_min)+1


#Repeat list for augmented data, flip=1
pl_peak_list_solo_aug =np.repeat(pl_peak_list_solo,2)
...
data_list_type = pl_peak_list_solo_aug
...
#Seeds are fixed to improve consistency for training. Data can be shuffled further by using KFold functionality later
seed_value = 1
seed(seed_value)
...
for train, test in kfold.split(inputs1,inputs2, targets):
    ...
    model_kfold.save(Results_path +'/Models' +"/KFoldB_" + Date + "_" + str(fold_no))
